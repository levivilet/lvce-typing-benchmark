...
test('writes the syntax highlight report and requested charts', async () => {
  ...
  const videosHtml = await readFile(join(output, 'videos.html'), 'utf8')
  ...
  assert.match(html, /href="\.\/videos\.html">Recorded loads<\/a>/)
  assert.doesNotMatch(html, /<video\b/)
  assert.match(videosHtml, /<h1>Recorded loads<\/h1>/)
  assert.match(videosHtml, /href="\.\/">Back to charts and tables<\/a>/)
  assert.match(videosHtml, /src="\.\/videos\/codemirror\.webm"/)
  assert.match(videosHtml, /Video capture is not included in the benchmark measurements/)
  assert.match(videosHtml, /CodeMirror/)
  assert.match(videosHtml, /v6\.0\.2/)
  assert.doesNotMatch(videosHtml, /<img\b|<table\b/)
